...
export async function main() {
    ...
    // the page unloads
    window.addEventListener('unload', async () => await socket.request('leave', { peerId: myPeerId }));
}
...
export async function joinRoom() {
    ...
    try {
        ...
        let routerRtpCapabilities = await socket.request('getRouterRtpCapabilities', { peerId: myPeerId });
        if (!device.loaded) {
...
export async function stopStreams() {
    ...
    let { error } = await socket.request('closeTransport', { peerId: myPeerId, transportId: sendTransport.id });
    if (error) {
        err(error);
    }
...
export async function pauseConsumer(consumer) {
    if (consumer) {
        ...
        try {
            await socket.request('pauseConsumer', { peerId: myPeerId, consumerId: consumer.id });
...
export async function resumeConsumer(consumer) {
    if (consumer) {
        ...
        try {
            await socket.request('resumeConsumer', { peerId: myPeerId, consumerId: consumer.id });
...
export async function pauseProducer(producer) {
    if (producer) {
        ...
        try {
            await socket.request('pauseProducer', { peerId: myPeerId, producerId: producer.id });
...
export async function resumeProducer(producer) {
    if (producer) {
        ...
        try {
            await socket.request('resumeProducer', { peerId: myPeerId, producerId: producer.id });
...
async function closeConsumer(consumer) {
    ...
    try {
        ...
        await socket.request('closeConsumer', { peerId: myPeerId, consumerId: consumer.id });
...
async function createTransport(direction) {
    ...
    let transport,
        { transportOptions } = await socket.request('createTransport', { peerId: myPeerId, direction });
    ...
    transport.on('connect', async ({ dtlsParameters }, callback, errback) => {
        let { error } = await socket.request('connectTransport', {
    ...
    if (direction === 'send') {
        ...
        transport.on('produce', async ({ kind, rtpParameters, appData }, callback, errback) => {
            ...
            // failure.
            let { error, id } = await socket.request('sendTrack', {
                transportId: transportOptions.id,
                kind,
                rtpParameters,
                paused,
...
export async function updatePeersDisplay(peersInfo = lastPollSyncData, sortedPeers = sortPeers(peersInfo)) {
    log('room state updated', peersInfo);

    $('#available-tracks').innerHTML = '';
    if (camVideoProducer) {
        $('#available-tracks').appendChild(
            makeTrackControlEl('my', 'cam-video', peersInfo[myPeerId].media['cam-video'])
        );
    }
    if (camAudioProducer) {
        $('#available-tracks').appendChild(
            makeTrackControlEl('my', 'cam-audio', peersInfo[myPeerId].media['cam-audio'])
        );
    }
    if (screenVideoProducer) {
        $('#available-tracks').appendChild(
            makeTrackControlEl('my', 'screen-video', peersInfo[myPeerId].media['screen-video'])
        );
    }
    if (screenAudioProducer) {
        $('#available-tracks').appendChild(
            makeTrackControlEl('my', 'screen-audio', peersInfo[myPeerId].media['screen-audio'])
        );
    }
    ...
    try {
        for (let consumer of consumers) {
            ...
            let mediaInfo =
                ...
                lastPollSyncData[consumer.appData.peerId].media[consumer.appData.mediaTag];
...
function makeProducerTrackSelector({ internalTag, container, peerId, producerId, currentLayer, layerSwitchFunc }) {
    try {
...
async function sig(endpoint, data, beacon) {
    try {
        ...
        let response = await fetch('/signaling/' + endpoint, { method: 'POST', body, headers });
